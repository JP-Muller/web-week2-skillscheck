...
let doubled = myNumbers.map((item)=> {
	return item *2
})
...
let filtered = myNumbers.filter((item)=>{
	return item > 100
})
...
let total = myNumbers.reduce((acc, cval) => {
	return acc+=cval
})
...
let forTheLoveOfGeorge = notGeorge.map((item) => {
	return item = 'George'
})
...
let enemies = people.filter((item)=> {
	return !item.friend
})
...
let totallyAwesome = people.reduce((acc, cval) => {
	return acc += cval.awesomeLevel
},0)
